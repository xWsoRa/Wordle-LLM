...
if "OPENAI_API_KEY" not in os.environ:
    os.environ["OPENAI_API_KEY"] = getpass.getpass("openapi key here")
...
def play_wordle(word):
    ...
    with open(r"path to wordle_history.txt", 'a') as history_file:
        while guess.lower() != word.lower() and attempts < 9:
...
def provide_feedback_presence(target_word, guess):
    ...
    for i in range(min_length):
        if guess[i] == target_word[i]:
            feedback.append(f"The letter {guess[i]} is in the correct position")  # Letter is in the correct position
        elif guess[i] in target_word and guess[i] != target_word[i]:
            feedback.append(f"The letter {guess[i]} is in the target word but in wrong position")  # Letter is present in the word but not in the correct position
        else:
            feedback.append(f"The letter {guess[i]} is not in the target word")  # Letter is not present in the word
...
def main():
    ...
    while games < 15:  # Alter the total number of games as needed
